...
def args_parser():
    ...
    parser.add_argument('--alpha', type=float, default=0.5, help="Dirichlet distribution alpha (0.1=extreme, 100=iid)")
    ...
    # === [新增] 全局数据使用率 ===
    parser.add_argument('--data_usage', type=float, default=1.0, help="global data usage ratio for training dataset (0.0 < x <= 1.0)")
    # ==========================
    ...
    parser.add_argument('--gamma', type=float, default=2.0, help='CrossFreeze parameter gamma for anchoring loss')
    parser.add_argument('--ld', type=float, default=1.0, help='FedProto parameter lambda for prototype alignment loss')
    parser.add_argument('--edit_ratio', type=float, default=0.07, help='pFedEdit: ratio of layers to edit (k)')
    parser.add_argument('--subset_ratio', type=float, default=0.1, help='pFedEdit: ratio of subset for evaluation (p)')
    parser.add_argument('--mu_moon', type=float, default=5.0, help='MOON parameter mu (weight of contrastive loss)')
    parser.add_argument('--temperature', type=float, default=0.5, help='MOON parameter temperature')

    # === [新增] 新算法参数 ===
    # FedPAC 参数
    parser.add_argument('--lam_pac', type=float, default=1.0, help='FedPAC: weight for prototype regularization')

    # DFSL (FedGKD) 参数
    parser.add_argument('--mu_dfsl', type=float, default=1.0, help='DFSL: weight for distillation loss')
    parser.add_argument('--temp_dfsl', type=float, default=2.0, help='DFSL: temperature for KL divergence')

    # MAML (FedMeta) 参数
    parser.add_argument('--alpha_maml', type=float, default=0.01, help='MAML: inner loop learning rate')
    parser.add_argument('--beta_maml', type=float, default=0.001, help='MAML: outer loop learning rate')
